...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: {
      ...
      zh: '给大模型推理服务做「性能体检」的工具，对着 vLLM 的 OpenAI 兼容接口跑。通过流式接口量出首 token 延迟、逐 token 间隔的 p50/p95/p99 和吞吐量，顺序、并发两种模式都支持。每次跑完和提交在仓库里的基线比，超阈值直接把 CI 卡红 —— 基线只允许 main 分支更新，PR 光对比不覆盖。自带 mock 推理后端，没 GPU 也能把整条流水线走通，还能跨 batch size、服务配置做 A/B 对比。',
    },
  ...
  },
  {
    ...
    description: {
      ...
      zh: '端到端求职助手技能 — 持久化候选人画像、多 Agent 深度 JD 分析、定制化简历（MD + DOCX）、针对性求职信、完整面试指南、间隔复习面试准备。双轴画像（知识轴 + 行为轴）随着每次交互持续进化。作为 Codex skill 构建，给 AI 编程助手用。',
    },
  ...
  },
  {
    ...
    description: {
      ...
      zh: '自己跑马需要教练，就自己写了一个。三个大模型（DeepSeek、Kimi、OpenAI）按场景自动路由，7 个教练技能模块各管一块（配速、体能、营养等），YAML 配置 + 共享状态。整条 Agent 流水线全自动跑：从 COROS 手表拉数据 → 运动科学模型推理 → 生成训练计划 → 推送到设备，一个完整的「观察-推理-行动」闭环。',
    },
